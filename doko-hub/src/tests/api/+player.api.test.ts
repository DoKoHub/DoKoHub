...
describe('API /api/player', () => {
    ...
    test('GET: Should return a list with existing players (Status 200)', async () => {
        ...
        await api.post('/api/player', { name: name2, ...MOCK_PLAYER_DATA_FULL, subject: 'sub2' });
    ...
    test('POST: Should fail if "name" is not a string (Status 400)', async () => {
        let response = await api.post('/api/player', { name: 12345, ...MOCK_PLAYER_DATA_FULL });
    ...
    test('POST: Should successfully create a new player with identity (Status 201)', async () => {
        ...
        const response = await api.post('/api/player', { name: playerName, ...MOCK_PLAYER_DATA_FULL });
...
describe('API /api/player/[player]', () => {
    ...
    beforeEach(async () => {
        await setupDatabase();
        const response = await api.post('/api/player', { name: INITIAL_PLAYER_NAME, ...MOCK_PLAYER_DATA_FULL });
