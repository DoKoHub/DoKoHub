<script lang="ts">
  ...
  // aktuell ausgewählter Tab in der Bottom-Navigation
  let activeTab: "groups" | "stats" | "profile" = "stats";

  // entscheidet, welche CSS-Klasse der Button bekommt
  function tabClass(tab: "groups" | "stats" | "profile") {
    return `bottom-nav-button ${activeTab === tab ? "active" : ""}`;
  }

  // Tab umschalten + ggf. Navigation
